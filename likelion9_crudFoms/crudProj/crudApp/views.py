...
#글쓰기 함수
def create(request): # Form과 관련된 view를 호출
    if request.method == "POST": # view에서 다시 한번 method를 확인
        form = PostForm(request.POST) # form의 유효성 검증 시작
        if form.is_valid():
            form = form.save(commit=False)
            form.pub_date = timezone.now()
            form.save() # form의 입력값 유효성 검증 끝
            return redirect('read') # form이 잘 입력되었다고 판단되면(valid 판정) 지정한 url(성공 url)로 이동
    else: # 잘 입력되지 않았을 경우 폼을 다시 입력 받기
        form = PostForm
